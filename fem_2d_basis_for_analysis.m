...
% as logrithmic unit sound pressure level (SPL) with 2e-5 Pa as reference 
% value (hearing threshold)
...
% interesting plots:
% # freq=34, Absorption degree low: very strong first axial room resonance called room mode)
% # freq=68, Absorption degree low: second axial mode, even with piston sound pressure does not increase significantly at source
...
% Model boundaries either by practial measured absorption coefficients (simpler) or by physical more correct acoustical impedances under assumption of local reaction
...
% either:
alpha = [0.1,0.1,0.1;
        0.2,0.1,0.1]; % absorption coefficent at wall, scattering object, piston casing (expresses: sound energy absorbed per reflection in percent at boundary, value range: ]0,1[)
...
video_time = 5; % set movie time in milliseconds
