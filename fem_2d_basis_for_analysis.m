...
S_e_average=L*W/Ne; % Average area of an element (approximation)
...
C(diagIdx(Boundary_vector) ) = beta; % beta/(rho0*c0); %% NN fehlt, *rho, kein c0
...
A = H/rho0 - w^2*Q/((rho0*c0^2)*(1+1i*air_damp))+1i*w*C/(rho0*c0); % (H - w^2*Q/(1+1i*air_damp)+1i*w*C);
b = F;
test=inv(w^2*Q);
max(abs(test(:,50)).^2)
min(abs(test(:,50)).^2)
